...
- The demo sets `DemoMode=true` (API) / `DEMO_MODE=true` (scraper) → tracker writes return 403 while AI analyses stay enabled (**allowlist middleware** in `Program.cs` / `main.py` — new mutating endpoints must be allowlisted to work in demo); the client reads `GET /api/config` for the read-only banner.
