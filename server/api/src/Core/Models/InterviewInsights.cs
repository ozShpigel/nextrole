...
// One-per-user, persisted observation feed — a standing, free-form summary
// Claude wrote after reading every interview retro together. Pure observation:
// no rubric shape, no adopt-into-Q&A action (decoupled from interview-prep by
...
{
    // The user's id IS the document id — same one-document-per-user shape as
    // ResumeFile and the profile. (Was a fixed "current" singleton before
    // multi-user.)
    [BsonId]
