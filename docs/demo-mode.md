...
- Two *persisting* writes are deliberately allowed anyway, both bounded to the seeded pool: **Generate Pack** (`POST /api/applications/{id}/pack`, path-pattern matched, capped by the shared `"pack"` rate limit) and the Matches page's **Add** (scraper `POST /api/discovery/jobs/{id}/save`, path-pattern matched; its downstream call to the API's `POST /api/applications` is gated on the `X-Source: ingest` header the scraper already attaches to every scraper→API request, so the same endpoint stays 403 for the client's "Import Job" feature — arbitrary URL scraping / uncapped Claude scoring has no such bound).
