...
- **Run the Greenhouse ingest locally against one small board.** The root `docker-compose.yml` defaults both Greenhouse services to `config/companies.dev.json` (one board, ~66 postings, roughly $0.45 a full run); `GREENHOUSE_COMPANIES_CONFIG=config/companies.json` runs the real list. A day of local runs over every board cost $6.25 on the local key — two thirds of that week's bill. Never shrink a board by capping postings instead: the close diff closes everything beyond the cap. Detail: `docs/greenhouse.md` → Running it.
