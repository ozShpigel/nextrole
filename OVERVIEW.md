...
- **CLI:** `python -m app.cli run-pool` ([`server/scraper/app/cli.py`](server/scraper/app/cli.py)); `dotnet run --project server/api/src/EvalHarness -- <verdict|subscore>`; `dotnet run --project server/api/src/Seeder`; `dotnet run --project server/api/src/DbCopy -- <src>=<dst>`.
