...
- **Environments:** local (`docker compose up`, or three dev processes), plus one VPS running both instances side by side — `nextrole.cloud` (public, `Identity:Mode=Cookie`) and `private.nextrole.cloud` (Basic-Auth frontend, `Identity:Mode=Fixed`) — as separate Compose services from the same images, differing only in env file. See [`deploy/compose.yml`](deploy/compose.yml).
