...
export function NoteList({ notes, onRefresh }: NoteListProps) {
  ...
  return (
    <>
      {notes.map((n) => (
        <div key={n.id} className="bg-muted border border-border rounded p-[1rem_1.25rem] mb-3 transition-all">
          ...
          <div className="text-[0.78rem] text-muted-foreground tabular-nums">{formatDateTime(n.createdAt)}</div>
